...
import { UrlTree, UrlSegment, DefaultUrlSerializer } from '@angular/router';
...
export class UrlSerializerService {
  ...
  parse(url: string): UrlTree {
    ...
    const lastSlashIndex = url.lastIndexOf("/"),
      equalSignIndex = url.indexOf("=", lastSlashIndex);
    if (equalSignIndex > -1) {
      // url contians '=', apply patch
      ...
      urlTree.root.children["primary"].segments.forEach(
        (segment: UrlSegment) => {
